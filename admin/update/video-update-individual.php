					...
					<?php
						if (isset($_POST['update'])) {
							...
							while ($row_lang = mysqli_fetch_array($result_lang)) {
								...
								$sql_title = "select * from video_title where video_id = $video_id";
								...
								while ($row_title = mysqli_fetch_array($result_title)) {
									...
									while ($row_path = mysqli_fetch_array($result_path)) {?>
										<form class="form-group" action="../include/contact.inc.php">
							...
							header("location:../index.php?update=successs");
						}
						elseif (isset($_POST['delete'])) {
							...
							while ($row_lang = mysqli_fetch_array($result_lang)) {
								...
								while ($row_title = mysqli_fetch_array($result_title)) {
									...
									$result_path = mysqli_query($conn,$sql_path);
									while ($row_path = mysqli_fetch_array($result_path)) {
										$sql_title_delete = "DELETE FROM `video_title` WHERE video_id = $video_id";
										$sql_path_delete = "DELETE FROM `video_path` WHERE video_path_id = $video_path_id";
										mysqli_query($conn,$sql_title_delete);
										mysqli_query($conn,$sql_path_delete);
									}
								}
							}
							header("location:../index.php?delete=successs");
						}
						else{
